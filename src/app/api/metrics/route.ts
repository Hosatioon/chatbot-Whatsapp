...
export async function GET(request: NextRequest) {
  try {
    ...
    const salesToday = (db.prepare(`
      ...
      WHERE tenant_id = ? AND status = 'DELIVERED' AND updated_at >= ? AND deleted_at IS NULL
    `).get(tenantId, fromTs) as { total: number }).total;

    const salesMonth = (db.prepare(`
      SELECT COALESCE(SUM(total_amount), 0) as total FROM orders
      WHERE tenant_id = ? AND status = 'DELIVERED'
      AND updated_at >= ? AND deleted_at IS NULL
    `).get(tenantId, Math.floor(new Date(now.getFullYear(), now.getMonth(), 1).getTime() / 1000)) as { total: number }).total;
    ...
    `).get(tenantId, fromTs) as { avg_time: number | null }).avg_time;
    ...
    const topProducts = db.prepare(`
      ...
      ORDER BY qty DESC
      LIMIT 5
    `).all(tenantId, fromTs) as { product_name: string; qty: number; revenue: number }[];
    ...
    const productsNoSales = db.prepare(`
      ...
      ORDER BY p.name ASC
    `).all(tenantId, tenantId, fromTs) as { name: string; stock: number; price: number }[];
    ...
    const totalActiveProducts = (db.prepare(`
    ...
    const recurringCustomers = (db.prepare(`
      SELECT COUNT(*) as count FROM (
    ...
    const topCustomers = db.prepare(`
      ...
      LIMIT 5
    `).all(tenantId, fromTs) as { customer_phone: string; customer_name: string | null; order_count: number; total_spent: number; last_order: number }[];
    ...
    const cancellationRate = totalOrders > 0
      ? Math.round((ordersCancelled / totalOrders) * 100)
      : 0;
    ...
    const salesByDay = db.prepare(`
      ...
      ORDER BY date DESC
      LIMIT 7
    `).all(tenantId, Math.floor((now.getTime() - 7 * 86400000) / 1000)) as { date: string; orders: number; revenue: number }[];
