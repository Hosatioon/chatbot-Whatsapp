/**
 * Cliente Baileys para el bot-service.
 * Maneja conexiones WebSocket con WhatsApp para cada tenant.
 */
...
async function start(tenantId: number): Promise<void> {
  ...
  try {
    ...
    console.warn(`[bot:${tenantId}] No se pudo obtener última versión:`, err);
  }
  ...
  sock.ev.on("connection.update", (update) => {
    ...
    if (qr) {
      console.log(`[bot:${tenantId}] QR recibido. Escanealo desde el dashboard.`);
    ...
    if (connection === "close") {
      ...
      if (code === DisconnectReason.loggedOut) {
        console.log(`[bot:${tenantId}] Sesión cerrada. Borrando auth...`);
        try {
  ...
  sock.ev.on("messages.upsert", async (event) => {
    ...
    try {
      ...
      console.error(`[bot:${tenantId}] Error procesando mensajes:`, err);
    }
  });
...
function scheduleReconnect(tenantId: number, code?: number): void {
  ...
  if (state.reconnectCount >= MAX_RECONNECT_RETRIES) {
    console.error(
      `[bot:${tenantId}] Máximo de reintentos (${MAX_RECONNECT_RETRIES}) alcanzado.`,
    );
    setConnectionState(tenantId, {
      status: "disconnected",
      qr_string: null,
